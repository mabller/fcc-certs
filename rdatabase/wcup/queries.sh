...
echo "$($PSQL "SELECT DISTINCT t.name FROM games g JOIN teams t ON t.team_id IN (g.winner_id, g.opponent_id) WHERE g.year = 2014 AND g.round = 'Eighth-Final' ORDER BY t.name;")"
